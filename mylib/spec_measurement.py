...
dir_data = '/home/minghao/Desktop/research/commonuse/mylib/mylib/data'
...
class Spectrum(lambdafunc):
    ...
    def flux(self, filt):
        ...
        return np.trapz(filtered_flux, filt.wavelength)\
    ...
    def magnitude(self, filt, style='AB'):
        ...
        stdspec = ((3631*u.Jy).cgs * const.c \
                / ((filt.wavelength*filt.units[0]).cgs)**2).to(self.units[1])

        stdflux = np.trapz(stdspec*filt.value, filt.wavelength)\
                * filt.units[0]
...
if __name__=='__main__':
    filt = read_filter('WISE', 'W2', uflag=2)
    wavelength = np.arange(20000, 80000, 1)

    stdspec = 3631e-23 * (29979245800) / wavelength / (wavelength*1e-8)
    print(stdspec)
    spec = Spectrum(wavelength, stdspec)
    print(spec.magnitude(filt))
